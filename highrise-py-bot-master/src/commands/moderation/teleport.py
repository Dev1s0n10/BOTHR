...
class Command:
    def __init__(self, bot):
        self.bot = bot
        self.name = "teleport"
        self.description = "Телепортировать игрока в определенную позицию"
        self.aliases = ['tp']
    ...
    async def execute(self, user: User, args: list, message: str):
        ...
        if len(args) < 2:
            ...
            try:
                ...
                await self.bot.highrise.send_whisper(user.id, "Неверные координаты")
                return
        else:
            position_name = " ".join(args[1:])
            if position_name not in self.room_positions.get("spawn", {}):
                await self.bot.highrise.send_whisper(user.id, f"{position_name} не является допустимой позицией в этой комнате.")
                return
            else:
